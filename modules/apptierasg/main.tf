resource "aws_launch_template" "alt" {
    name = "${var.project-name}-alt"
    image_id = var.ami
    instance_type = var.instance-type
    key_name = var.key-name
    user_data = filebase64("../modules/apptierasg/config.sh")  
    vpc_security_group_ids = [var.app-tier-sg-id]
    tags = {
      Name = "${var.project-name}-alt"
    }
}
...
resource "aws_autoscaling_group" "asg" {
  ...
  desired_capacity          = 4
  vpc_zone_identifier = [var.private-3-1a-id,var.private-4-1b-id]
